...
public enum Op : byte
{
    ...
    LOOT_LOCK_REQUEST = 27, // 루팅 락 요청
    LOOT_LOCK_STATE = 28, // 루팅 락 상태
    LOOT_UNLOCK = 29, // 루팅 락 해제
    DEAD_PLAYER_LOOT_REQUEST = 30, // 죽은 플레이어 루팅 요청
    AI_EVENT_BROADCAST = 31, // AI 이벤트 브로드캐스트 (시간 기준)
    ROOM_LIST_REQUEST = 32, // 방 리스트 요청
    ROOM_LIST_RESPONSE = 33, // 방 리스트 응답
    ROOM_CREATE = 34, // 방 생성
    ROOM_JOIN = 35, // 방 참가
    ROOM_PASSWORD = 36, // 방 비밀번호
    RECONNECT_STATE = 37, // 재접속 상태 복구
    MOD_EVENT = 38, // 모드 이벤트 (API용)
    BATCH_UPDATE = 39, // 배치 업데이트 (문, 파괴물 등)
    ZONE_BROADCAST = 40, // 구역 기반 브로드캐스트
    DEBUG_INFO_REQUEST = 41, // 디버그 정보 요청
    DEBUG_INFO_RESPONSE = 42, // 디버그 정보 응답
